...
- Treat [spec/openapi.json](spec/openapi.json) (REST endpoint schemas) and [spec/events.schema.json](spec/events.schema.json) (SSE event schema) as the **primary, maximum-authority sources** for the remote API contract — they come directly from the backend. Names/casing already match this SDK; no snake_case-to-camelCase translation is needed.
- Treat the `docs/*.md` guides (see README.md's Documentation table for the module-to-file mapping) as the secondary, SDK-usage-level description of that contract, and `/memories/repo/` notes as tertiary. Cross-check any "missing", "TODO", or "not yet implemented" claim against `spec/openapi.json` and live code/`tests/` before citing it.
