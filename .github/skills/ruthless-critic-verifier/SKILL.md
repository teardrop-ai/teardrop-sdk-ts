...
- [ ] Claimed API "gaps" or "not implemented" notes are reproduced against current SDK code, not assumed from `spec/openapi.json`/`spec/events.schema.json` alone — those specs are authoritative for the *remote API contract* but may describe endpoints not yet wired into a client method.
- [ ] New/changed public behavior is documented in the matching `docs/*.md` guide (README.md is a lean index/nav table only, not a place for detailed usage docs) — check README.md's Documentation table for the correct module-to-file mapping, and confirm a new module's doc file is listed there.