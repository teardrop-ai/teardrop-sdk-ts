...
- `EventTriggersModule.fire()` sends a plain secret header (`X-Teardrop-Trigger-Secret`) that the *server* compares; this SDK has no HMAC signing/verification helper and should not invent one unless the API contract in [03_SDK_HANDOFF.md](notes/03_SDK_HANDOFF.md) changes.
